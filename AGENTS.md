...
- Running `:todo` should show open Markdown task lines from all notes in a scrollable launcher palette that mirrors the full-text search results layout, with the note name and a color-coded note-level task completion percentage on the first line and the task text dimmed below it.
...
- Pressing `Ctrl+A` in the editor should discard unsaved changes and return to the launcher.
